		...
		const onFetchError = (error) => {
			...
			if (errorCode === NOT_FOUND) {
				...
				invoiz.showNotification({
					message: format(filteredError, resources.str_offerUpperCase),
					type: "error",
				});
			} else {
				invoiz.showNotification({ message: resources.defaultErrorMessage, type: "error" });
			}
			invoiz.router.navigate(`/proformaInvoices`);
		};

		fetchOfferData().then(whenRequestsDone).catch(onFetchError);
	}
