...
class SettingsDunningsComponent extends React.Component {
	...
	getReminderBlock(levelName, leftHeaderText, textModulePlaceholder) {
		...
		return (
			<div className="row dunningSettings_block">
				<div className="col-xs-4 form_groupheader_edit text-h4">{leftHeaderText}</div>
				<div className="col-xs-8">
					<div className="u_mt_8">
						{ canUpdateDunning ? <div className="dunningSettingsActive_toggle">
							<OvalToggleComponent
								labelLeft
								onChange={val => {
								...
								labelText={format(resources.settingsDunningsActivatePaymentReminder, leftHeaderText)}
								newStyle={true}
								disabled={true}
							/>
						</div> : null }						
					...
					<div className="dunningSettingsDays">
						...
						<div className="dunningSettingsDays_input">
							<TextInputComponent
								...
								onChange={evt => this.updateValue(levelName, 'daysTillAlert', Number(evt.target.value))}
								autoComplete="off"
								spellCheck="false"
						...
						<div>{leftHeaderText !== resources.str_paymentRemainder ? resources.settingsDunningsDaysAfterLastReminder : resources.settingsDunningsDaysAfterDue}</div>
					</div>
					...
					<div className="col-xs-offset-6">
						<a
							...
							onClick={evt => this.onToggleTextModulesClick(`textModules_${levelName}`, evt.nativeEvent)}
						>
	...
	onToggleTextModulesClick(ref, event) {
		...
		if (target[0] && textModules) {
			const linkCaption = ($(textModules).is(':visible') ? ` ${resources.str_show}` : ` ${resources.str_hideTitle}`) + ' ' + resources.str_textModulesSmall;
				...
				.then(results => {
					...
					if (results[0].state === 'rejected') {
						...
						invoiz.page.showToast({
							type: 'error',
							message: format(resources.settingsDunningsSaveErrorMessage, resources.str_paymentRemainder)
						});
					}
					...
					if (results[1].state === 'rejected') {
						...
						invoiz.page.showToast({
							type: 'error',
							message: format(resources.settingsDunningsSaveErrorMessage, resources.settingsDunningsFirstReminder)
						});
					}
					...
					if (results[2].state === 'rejected') {
						...
						invoiz.page.showToast({
							type: 'error',
							message: format(resources.settingsDunningsSaveErrorMessage, resources.settingsDunningsSecondReminder)
						});
					}
					...
					if (results[3].state === 'rejected') {
						...
						invoiz.page.showToast({
							type: 'error',
							message: format(resources.settingsDunningsSaveErrorMessage, resources.settingsDunningsLastReminder)
						});
					}
					...
					if (!error) {
						...
						if (!errorPaymentReminder) {
							invoiz.page.showToast({
								message: format(resources.settingsDunningsSaveSuccessMessage, resources.str_paymentRemainder)
							});
						}
						if (!errorFirstReminder) {
							invoiz.page.showToast({
								message: format(resources.settingsDunningsSaveSuccessMessage, resources.settingsDunningsFirstReminder)
							});
						}
						if (!errorSecondReminder) {
							invoiz.page.showToast({
								message: format(resources.settingsDunningsSaveSuccessMessage, resources.settingsDunningsSecondReminder)
							});
						}
						if (!errorLastReminder) {
							invoiz.page.showToast({
								message: format(resources.settingsDunningsSaveSuccessMessage, resources.settingsDunningsLastReminder)
							});
						}
					}
				})
					...
					<div className="row u_pb_40">
						<div className="col-xs-12">
							...
							<div className="text-muted">
								<p>{resources.settingsDunningsSetReminderText}</p>
								<p>{resources.settingsDunningsSetAmountText}</p>
							</div>
						</div>
					</div>

					{this.getReminderBlock('paymentReminder', resources.str_paymentRemainder, resources.settingsDunningsThePaymentReminder)}
					{this.getReminderBlock('firstReminder', `1. ${resources.str_warning}`, resources.str_firstReminder)}
					{this.getReminderBlock('secondReminder', `2. ${resources.str_warning}`, resources.str_secondReminder)}
