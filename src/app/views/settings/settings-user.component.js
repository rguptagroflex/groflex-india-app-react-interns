...
import { isZohoSubscriber, canExtendUsers, isChargebeeSubscriber, isRazorpaySubscriber } from 'helpers/subscriptionHelpers';
...
class SettingsUserComponent extends React.Component {
	constructor(props) {
		...
		const { currentUserCount, maxUserCount, planId, currentPaidUsers, pendingSeatInvites, currentAddons, tenantId } = invoiz.user.subscriptionData;
			...
			this.setState(
				...
				() => {
					updateSubscriptionDetails(() => {
						const { currentPaidUsers, maxUserCount, currentUserCount, currentAddons, pendingSeatInvites } = invoiz.user.subscriptionData;
						this.setState(
							{
								pendingSeatInvites,
								currentAddons,
	...
	updateUserRole(user, selectedRole) {
		...
		if (selectedRole === multiUserRoleType.OWNER) {
				...
				.then(() => {
					updateUserPermissions(() => {
						this.setState(
							{
								canDeleteUser:
									invoiz.user && invoiz.user.hasPermission(userPermissions.DELETE_USER),
								canChangeAccountOwner:
									invoiz.user &&
									invoiz.user.hasPermission(userPermissions.CHANGE_ACCOUNT_OWNER),
								canInviteUser:
									invoiz.user && invoiz.user.hasPermission(userPermissions.INVITE_USER),
							},
							() => {
			...
			invoiz.request(`${config.resourceHost}user/${user.id}/role`, {
				auth: true,
				method: 'POST',
				data: {
					role: selectedRole
				...
				this.updateUserData();
				invoiz.showNotification({message: `${user.firstName} ${user.lastName}'s role has been updated to ${this.getRoleTitle(selectedRole)}`});
			}).catch((err) => {
				console.log(err);
			});
		}
	...
	fetchUserList() {
		...
		return new Promise((resolve, reject) => {
			invoiz.request(`${config.settings.endpoints.getUserList}`, { auth: true })
				.then((response) => {
					const {
						body: {
							data: { roles }
	...
	getCABox() {
		const { canDeleteUser, canInviteUser, canChangeAccountOwner, otherRoles, maxUserCount, currentPaidUsers, planId, caRole } = this.state;
		...
		if (caRole.length > 0 && planId !== chargebeePlan.TRIAL && planId !== chargebeePlan.FREE_MONTH ) {
							...
							return (
								<div className='' key={`settings-role-container-${role.role}-${index}`} >
									...
									<div className='settings-role-other-users'>
										{role.users.map((user) => {
											const { firstName, lastName, email, hasConfirmedEmail, isCurrent, isExpired, id } = user;
											...
											return (
												<div
													className={`settings-user-box user-existing ${hasConfirmedEmail ? 'accepted' : 'pending'}`}
													key={`user-key-${
														hasConfirmedEmail ? `${firstName}.${lastName}` : `${title.replace(' ', '.')}`
													}-${index}`}
												>
													{(canDeleteUser ||
															(canInviteUser && !hasConfirmedEmail) ||
															(canChangeAccountOwner && hasConfirmedEmail) || isCurrent) && (
														<div>
															<div
																className={`popover-icon icon icon-menu icon-rotate-90`}
																id={`settings-user-box-popover-${id}`}
															/>
															{this.createPopover(`settings-user-box-popover-${id}`, user, true, isCurrent)}
														</div>
													)}

													<div className="settings-user-box-content">
														<div className="icon-wrapper">
															<div className={`icon icon-${hasConfirmedEmail ? `icon icon-${role.role}_user` : 'offen'}`} />
														...
														<div className="user-box-title text-semibold" id={`user-name-${index}`}>
															{title}
															{isCurrent && <div className="user-box-current-user">(You)</div>}
														</div>
														<div className="user-box-subtitle u_mt_6">{email}</div>
			...
			return (
				canInviteUser && planId !== chargebeePlan.FREE_MONTH && planId !== chargebeePlan.TRIAL && caRole.length === 0 && ( 
					<div>
						<div className="text-semibold u_mt_20 u_mb_20">Invite CA</div>
						<div
							className="settings-user-box  user-new"
							 onClick={() => this.inviteUser(true)}
						>
							<div className="settings-user-box-content">
			...
			return (
				<div className='settings-role-other-wrapper'>
					{modifiedroles.map((role, index) => {
						if (role && role.users && role.users.length > 0) {
							return (
								<div className='settings-role-other-container' key={`settings-role-container-${role.role}-${id}`} >
									<div className='settings-role-other-box-title text-semibold'>
										{this.getRoleTitle(role.role)}
									</div>
									<div className='settings-role-other-users'>
											...
											return (
												...
												>
													{(canDeleteUser ||
														...
														<div>
															<div
																...
																id={`settings-user-box-popover-${id}`}
															/>
															{this.createPopover(`settings-user-box-popover-${id}`, user, false, isCurrent)}
														</div>
													)}
													...
													<div className="settings-user-box-content">
														...
														<div className="user-box-title text-semibold" id={`user-name-${id}`}>
															{title}
															{isCurrent && <div className="user-box-current-user">(You)</div>}
														</div>
														<div className="user-box-subtitle u_mt_6">{email}</div>
	...
	inviteUser(inviteCAOnly) {
		const { currentUserCount, maxUserCount, canInviteUser, isLoading, canExtendUsers, showOldPlans, currentPaidUsers } = this.state;
		const { resources } = this.props;
	...
	buyUser() {
		const { currentUserCount, maxUserCount, canInviteUser, isLoading, canExtendUsers, showOldPlans, currentPaidUsers, planId, tenantId, pendingSeatInvites } = this.state;
		const { resources } = this.props;
	...
	getPaidUsers() {
		...
		if (pendingSeatInvites > 0) {
			for (let i = 1; i <= pendingSeatInvites; i++) {
				inviteUserFields.push(
					canInviteUser && maxUserCount !== currentPaidUsers && planId !== (chargebeePlan.FREE_MONTH || chargebeePlan.TRIAL) && canDeleteUser && (
		...
		return (
			<div className="settings-user-paid-list">
				{inviteUserFields}
			</div>
		);
	}
	...
	render() {
		const { currentUserCount, maxUserCount, canInviteUser, canExtendUsers, planId, currentPaidUsers, otherRoles, currentAddons, pendingSeatInvites } = this.state;
		const showPriceInformation = canInviteUser && canExtendUsers && maxUserCount && maxUserCount <= currentUserCount;
		return (
				...
				{this.state.isLoading ? (
					...
					<div className="settings-user-content">
						...
						<div className="settings-user-paid">
								...
								{
									canInviteUser && pendingSeatInvites > 0 ? <div className='settings-role-other-box-title text-semibold' style={{fontSize: 20}}>
							...
							<div className="settings-user-other">
								...
								{planId !== chargebeePlan.FREE_MONTH && planId !== chargebeePlan.TRIAL ? (
									<div style={{marginBottom: 20}}>	
										<div className="text-semibold u_mt_20 u_mb_10" style={{fontSize: 20}}>Team members</div>							
										{									
											(currentPaidUsers > 0 || pendingSeatInvites > 0) ? (
												<div>																						
													<span>Seat capacity: {currentPaidUsers} of {maxUserCount}</span>
													<br></br>
												</div>
											) : (canInviteUser ? <span>Begin adding team members by buying seats and inviting users</span> : <span>Members appear here when the owner invites them</span>)									
										}								
										{/* <span>Where {currentPaidUsers} is the current number of users.</span> */}
									</div>
								) : <div>
									{planId === chargebeePlan.TRIAL ? <div className="text-semibold u_mt_20 u_mb_30" style={{fontSize: 20}}>{`Please upgrade your trial plan to a paid plan to buy and invite additional users`}</div> : 
										<div className="text-semibold u_mt_20 u_mb_30" style={{fontSize: 20}}>{`Please upgrade your Free monthly plan to a paid plan to buy and invite additional users`}</div>}														
									<ButtonComponent
										type="primary"
