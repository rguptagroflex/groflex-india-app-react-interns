...
const QuickLinkItem = SortableElement((props) => {
	...
	const modIcon = require(`assets/images/svg/quick-links/${item.linkId}-modified.svg`);
	return (
		<div className="col-xs-2 text-center quick-link-item" onClick={() => invoiz.router.navigate(item.link)}>
			<span>
				{/* <SVGInline width="88px" height="auto" svg={icon} /> */}
				<SVGInline width="25px" height="auto" svg={modIcon} />
			</span>
			<p>{item.name}</p>
		</div>
	);
...
const QuickLinkList = SortableContainer((props) => {
	...
	return (
		<div className="row quick-link-row">
			{items.map((item, index) => (
				<QuickLinkItem key={index} index={index} value={item} disabled={disabled} {...restProps} />
			))}
		</div>
	);
});
	...
	async fetchQuickLinks() {
		try {
			...
			if (!this.state.canViewExpense) {
				links = links.filter(
					(x) =>
						x.linkId != "create-purchase-orders" && x.linkId != "add-expenses" && x.linkId != "add-stocks"
				);
			}
			...
			<div className="start-quick-links-container">
				<div className="widgetContainer box">
					<div style={{ marginBottom: "20px" }}>
						<p className="text-h5" style={{ display: "inline", marginTop: 0 }}>
							Quick Links
						</p>
						<div
							onClick={async () => await this.toggleQuickLinksEditable()}
							style={{ display: "inline-block", marginTop: 0, float: "right" }}
						>
							<p
								style={{
									// color: "#0079B3",
							...
							>
								<span>{!quickLinksEditable ? "Edit Links" : "Save"}</span>
								<SVGInline
									width="20px"
									height="15px"
