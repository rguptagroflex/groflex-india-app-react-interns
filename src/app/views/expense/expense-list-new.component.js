...
class ExpenseListNewComponent extends React.Component {
	...
	getReceiptsRenderer(receipts, width, number) {
		return ` ${
			receipts.length > 0
				? `<div class="cell-status-icon"><div class='icon icon-attachment' width="${width}"></div>`
				: ""
		} ${number ? `<span class='cell-status-icon-text'>${number}</span>` : ""}</div>`;
	}
					...
					<ListAdvancedComponent
						headTabbedFilterItemsFunc={(expenses) => {
							return [
								...
								},
								{
									...
									label: "Paid - Bank",
									count: expenses.filter((expense) => expense.payKind === "bank_paid").length,
								},
								{
									filter: {
										field: "payKind",
									...
									label: "Paid - Cash",
									count: expenses.filter((expense) => expense.payKind === "cash_paid").length,
								},
								{
									filter: {
										field: "status",
									...
									label: "Cancelled",
									count: expenses.filter((expense) => expense.status === "cancelled").length,
								},
								{
									filter: {
										field: "type",
									...
									label: "Expenses",
									count: expenses.filter((expense) => expense.type === "expense").length,
								},
								{
									filter: {
										field: "type",
							...
							},
							{
								...
								filterParams: {
									...
									valueFormatter: (evt) => {
										...
										if (evt.value.includes("paid")) return "Paid - Bank";
										if (evt.value.includes("cash")) return "Paid - Cash";
									},
						...
						responseDataMapFunc={(expenses) => {
							expenses = expenses.map((expense) => {
								...
								console.log(
									moment().diff(moment(expense.payDate + "T00:00:00.000Z"), "days"),
									expense.payDate
								);
								expense.payee = expense.displayName;
								expense.dueSince =
									expense.status === "open" ? moment().diff(expense.date, "days") || null : null;
								return expense;
							});
							return expenses;
						}}
