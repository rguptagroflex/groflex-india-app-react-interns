...
class ReceiptDetailComponent extends React.Component {
	constructor(props) {
		super(props);
	...
	// handleRatingModal() {
	// 	const triggerType = this.props.triggerType;
	// 	ModalService.open(
	...
	// 		}
	// 	);
	// 	$('.modal-base-view').addClass('narrow-modal');
	// }
	...
	addPayment() {
		...
		ModalService.open(
			<PaymentCreateModalComponent payment={payment} resources={this.props.resources} dunning={dunning} onSave={() => invoiz.router.reload()} />,
			{
				width: 600,
	...
	createPaymentTable() {
		...
		if (this.state.invoice.payments) {
			this.state.invoice.payments.forEach((payment) => {
				...
				cells.push({
					value: !payment.cancellationPaymentId ? (
						<IconButtonComponent
							icon="icon-close"
							size="medium"
