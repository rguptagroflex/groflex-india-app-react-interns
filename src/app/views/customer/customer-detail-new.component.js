...
class CustomerDetailNewComponent extends React.Component {
	constructor(props) {
		...
		const totalBalanceSum =  this.state.customer.salesOrExpensesVolumeData.outstandingAmount + parseInt(this.state.customer.credits) + this.state.customer.balance + this.state.customer.openingBalance;
		this.salesOrExpensesData = [
			{
				...
				count: customer.type === `customer` ? this.state.customer.salesOrExpensesVolumeData.invoiceCount : this.state.customer.salesOrExpensesVolumeData.expenseCount,
			},
			{
				title: customer.type === `customer` ? totalBalanceSum > 0 ? `Outstanding receivable` : totalBalanceSum === 0 ? `Balance` : `Outstanding payable` : `Open expenditures`,
				amount: customer.type === `customer` ? this.state.customer.balance + this.state.customer.credits : 
				this.state.customer.salesOrExpensesVolumeData.openExpensesTurnOver + this.state.customer.salesOrExpensesVolumeData.totalPurchasesTurnOver,
				count: customer.type === `customer` ? null : this.state.customer.salesOrExpensesVolumeData.openExpensesCount,
			},
			{
			...
			},
			{
				...
				amount: customer.type === `customer` ? this.state.customer.salesOrExpensesVolumeData.openOffersTurnOver : this.state.customer.salesOrExpensesVolumeData.openPurchaseOrdersTurnOver,
				count: customer.type === `customer` ? this.state.customer.salesOrExpensesVolumeData.openOffersCount : this.state.customer.salesOrExpensesVolumeData.openPurchaseOrdersCount,
			},
		];
	...
	getSalesOrExpenses() {
		...
		let openInvoicesOrExpensesSum = customer.type === `customer` ? formatCurrency(customer.salesOrExpensesVolumeData.outstandingAmount) : 
		formatCurrency(customer.salesOrExpensesVolumeData.openExpensesTurnOver + customer.salesOrExpensesVolumeData.totalPurchasesTurnOver)
		let creditsOrDebitsSum = customer.type === `customer` ? formatCurrencyMinusPlus(this.state.customer.credits) : formatCurrencyMinusPlus(this.state.customer.debits)
		let totalBalanceSum = customer.type === `customer` ? formatCurrencyMinusPlus(customer.salesOrExpensesVolumeData.outstandingAmount + parseInt(this.state.customer.credits) + this.state.customer.balance + this.state.customer.openingBalance) : 
		formatCurrencyMinusPlus(customer.salesOrExpensesVolumeData.openExpensesTurnOver + customer.salesOrExpensesVolumeData.totalPurchasesTurnOver + this.state.customer.debits + this.state.customer.balance + this.state.customer.openingBalance)
		const balanceSumValue = customer.salesOrExpensesVolumeData.outstandingAmount + parseInt(this.state.customer.credits) + this.state.customer.balance + this.state.customer.openingBalance
		...
		if (category.title) {
			return (
				<div
					...
					className=""
				>
					{customer.type === `customer` && (category.title === `Outstanding payable` || category.title === `Outstanding receivable` || category.title === `Balance`) ?
					<div className="balance-field"> 
					<div className="text-h6 u_mb_10">
						{category.title} 
					</div>
					</div> 
					: 
					...
					<div className="row">
						<div className="col-xs-7 col-gutter-right-30">
							...
							<div className={`text-h4 text-primary text-truncate`}>{`${totalBalanceSum}`}{<SVGInline height="21px" width="26px" svg={`${balanceSumValue > 0 ? PayIncoming : balanceSumValue === 0 ? '' : PayOutgoing}`} />}</div>
						</div>
						<div className="col-xs-6 col-gutter-right-30 u_mt_12">
							<div className="text-muted text-medium u_mb_6">Opening Balance</div>
							<div className="text-h4 text-primary text-truncate border-right u_mr_10">{formatCurrencyMinusPlus(customer.openingBalance)}</div>
						</div>
						<div className="col-xs-5 col-no-gutter-left u_mt_12" >
							<div className="text-muted text-medium u_mb_6">Excess payments</div>
							<div className="text-h4 text-primary text-truncate">{formatCurrencyMinusPlus(customer.balance)}</div>
						</div>
						<div className="col-xs-6 col-gutter-right-30 u_mt_12 box-clickable" onClick={() => {
						this.onSalesOrExpensesClick(balanceSubtitleRow1, category.count);
							...
							<div className="text-h4 text-primary text-truncate border-right u_mr_10">{openInvoicesOrExpensesSum}</div>
							</div>
						<div className="col-xs-5 col-no-gutter-left u_mt_12 box-clickable" onClick={() => {
	...
	onSalesOrExpensesClick(category, count) {
		let url, webStorageKey, settings, currentFilter, orderBy, values = [], sort = [], customerName, filterItems = {};
	...
	bulkPayment() {
		...
		const {  displayName, customerId,salesOrExpensesVolumeData:{outstandingAmount}, openingBalance } = customer;
		const openAmount = openingBalance > 0 
			...
			ModalService.open(
				...
				customer={customer} handlePaymentChange={handlePaymentChange }  invoiceOutstandingAmount={outstandingAmount}
				payment={payment} resources={resources}  onSave={() => invoiz.router.reload()} />,
				{
					width: 600,
	...
	bulkRefund() {
		...
		const { customer}=this.state
		const {resources}=this.props
		const {  displayName, customerId, openingBalance,credits,balance,salesOrExpensesVolumeData:{outstandingAmount}, } = customer;
		 ...
		 if(amount > 0 ){
		   ...
		   const data = {
			...
			totalDues,
			previousBalance,
			isClearingDues, clearingDues,
			isClearingInvoiceAmount, clearingInvoiceAmount
	...
	render() {
		...
		return (
			<div className="customer-detail-wrapper wrapper-has-topbar">
				...
				<div className="row">
					<div className="col-xs-4 col-no-gutter-left col-gutter-right-30">
						<CustomerMetadataComponent customer={customer} bulkPayment={this.bulkPayment} issueRefund={this.bulkRefund} />
