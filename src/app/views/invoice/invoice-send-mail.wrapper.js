...
class InvoiceSendMailWrapper extends React.Component {
    constructor(props) {
		super(props);
    ...
    preFetch ()  {
		...
		const showEmailView = ([invoiceResponse, textModuleResponse]) => {
			const {
				body: {
		...
		const onFetchError = error => {
			invoiz.router.navigate(`${config.invoice.resourceUrl}/${parseInt(id, 10)}`);
...
export default connect(mapStateToProps)(InvoiceSendMailWrapper);
