...
const buildLetterHeaderStyles = ({ font, fontSize, fontWeight, italic, underline, color }) => {
	return {
		fontFamily: `"${font}"`,
...
const fabricObjectToImageLetterElement = (fabricObject, letterElement) => {
	letterElement = letterElement instanceof LetterElement ? letterElement : new LetterElement();
...
const fabricObjectToTextLetterElement = (fabricObject, letterElement) => {
	...
	const fontColor = fabricObject.getFill() === 'rgb(0,0,0)' ? '#000000' : fabricObject.getFill();
	...
	letterElement.metaData.underline = fabricObject.getTextDecoration() === 'underline';
...
const fabricObjectToShapeLetterElement = (fabricObject, letterElement) => {
	...
	const shapeColor = fabricObject.getFill() === 'rgb(0,0,0)' ? '#000000' : fabricObject.getFill();
...
const buildText = function buildText(letterElement) {
	...
	const text = new LetterFabricText(letterElement.metaData.html, getTextOptions(letterElement));
