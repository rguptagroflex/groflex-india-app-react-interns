...
export const redirectToRazorpay = async (planId, subscriptionDetail) => {
     const checkoutResult = await loadRazorpayScript('https://checkout.razorpay.com/v1/checkout.js')
   //const checkoutResult = await loadRazorpayScript('https://checkout.razorpay.com/v1/razorpay.js') Possibly can be used in the future for configuring specific payment methods
	...
	if (planId) {
		...
		url =
			config.account.endpoints.getRazorpayCheckoutPage +
			'/' +
			planId
	}
		invoiz
			...
			.then(response => {
                ...
                const options = {
                    ...
                    "handler": async function(response) {
                        ...
                        setTimeout(() => {
                            invoiz.request(config.account.endpoints.verifyRazorpayTransaction, {
                                method: 'POST',
                                auth: true,
                                data
                            }).then((res) => {
