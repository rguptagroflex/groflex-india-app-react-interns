...
export const fetchImpressOfferData = options => {
	...
	invoiz
		...
		.then(({ body: { data } }) => {
			...
			if (data.offer.customerId && data.offer.customerData && Object.keys(data.offer.customerData).length > 0) {
				data.offer.customerData.id = data.offer.customerId;
			}

			if (impressData && impressData.theme && impressData.theme.items && impressData.theme.items.length === 5) {
				impressData.theme = JSON.parse(JSON.stringify(impressThemes.find(theme => theme.title === 'Standard')));
			...
			const offerData = {
				...
				globalSettings: {
					...
					positionsBlockExists: !!(impressData && impressData.positionsBlockExists)
				},
				pages: impressPages
			};

			if (offerData.standardOfferData && offerData.standardOfferData.positions) {
			...
			invoiz
				.request(`${config.resourceHost}impress/${options.offerId}/pages/${offerData.pages[0].id}`, {
					auth: true
				})
