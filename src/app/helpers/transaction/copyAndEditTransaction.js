...
const { TRANSACTION_TYPE_INVOICE, TRANSACTION_TYPE_RECURRING_INVOICE } = transactionTypes;
...
export const copyAndEditTransaction = opts => {
	...
	const onCopySuccess = response => {
		...
		if (type === OfferTypes.IMPRESS) {
			invoiz.router.navigate(`${transactionKind.toLowerCase()}/impress/edit/${id}`);
	...
	const onCopyError = error => {
		...
		if (error.body.meta.offer) {
			...
			invoiz.showNotification({ type: 'error', message: getResource([`${transactionKind}CopyErrorMessage`]) });
		}
	};
	...
	if (transactionKind === TRANSACTION_TYPE_RECURRING_INVOICE && !opts.invoiceModel.recurrence) {
		transactionKind = TRANSACTION_TYPE_INVOICE;
	}

	invoiz
		.request(`${config.resourceHost}${transactionKind}/${opts.invoiceModel.id}/copy`, {
			auth: true,
			method: 'POST'
		})
		.then(onCopySuccess)
