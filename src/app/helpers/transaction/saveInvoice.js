...
export const saveInvoice = (requestData, isDeposit, isClosing) => {
	...
	if (requestData.customerData.countryIso === "IN" && !requestData.baseCurrency && !requestData.exchangeRate) {
		requestData.baseCurrency = ''
		requestData.exchangeRate = 0.0
	}
	return new Promise((resolve, reject) => {
		const url = requestData.id
			? `${config.resourceHost}${isDeposit ? 'depositInvoice' : isClosing ? 'closingInvoice' : 'invoice'}/${
				requestData.id
			  }`
			: `${config.resourceHost}${isDeposit ? 'depositInvoice' : isClosing ? 'closingInvoice' : 'invoice'}`;
		...
		const dunningRequestData = {
			autoDunningEnabled: requestData.payConditionData.dueDays > 0 ? requestData.autoDunningEnabled : false,
