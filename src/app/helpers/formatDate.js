...
export const formatDate = (date, originalFormat, expectedFormat) => {
	return moment(date, originalFormat || config.dateFormat.api).format(expectedFormat || config.dateFormat.client);
};

export const formatDateLong = date => {

	const weekdayShort = getResource('weekdayNames')[new Date(date).getDay()].substr(0, 2);
...
export const formatApiDate = (date, originalFormat) => {
	...
	return moment(date, originalFormat || config.dateFormat.client).format(config.dateFormat.api);
};
...
export const formatClientDate = (date, originalFormat) => {
	...
	return moment(date, originalFormat || config.dateFormat.api).format(config.dateFormat.client);
};
...
export const formateClientDateMonthYear = (date, originalFormat) => {
	...
	return moment(date, originalFormat || config.dateFormat.api).format('DD-MM-YY');
};
