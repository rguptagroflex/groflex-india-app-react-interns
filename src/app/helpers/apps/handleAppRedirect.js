...
export const handleAppRedirect = (app) => {
	...
	switch (app.attributes.invoizAppType) {
		...
		case AppType.SLACK:
			invoiz
				.request(`${config.resourceHost}slack/incoming-webhook`, { method: 'GET', auth: true })
				.then((res) => {
					const {
					...
					if (!incomingWebhooks || incomingWebhooks.length === 0) {
						...
						ModalService.open(<SlackConnectionModalComponent channels={incomingWebhooks} />, {
		...
		case AppType.DEFAULT:
			if (app.customData.callbackUrl.indexOf('?pendo') !== -1) {
				...
				invoiz.router.navigate(window.location.pathname + callbackUrlPendo, true, true);
