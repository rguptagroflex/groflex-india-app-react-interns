...
class NumerationConfigComponent extends React.Component {
	constructor(props) {
		...
		this.state = {
			currentView:
				this.props.pathName == "/settings/more-settings/offer"
					? OFFER_VIEW
					: this.props.pathName == "/settings/more-settings/invoice"
					? INVOICE_VIEW
					: this.isOnlyInvoice
					? INVOICE_VIEW
					: this.isOnlyPurchaseOrder
					? PURCHASE_ORDER_VIEW
					: OFFER_VIEW,
			...
			purchaseOrderNumber:
				parseInt(purchaseOrderOptions.currentValue) + parseInt(purchaseOrderOptions.increment) || "",
		...
		setTimeout(() => {
			const datePart =
				this.state.currentView === INVOICE_VIEW
					? invoiceOptions.datePart
					: this.state.currentView === PURCHASE_ORDER_VIEW
					? purchaseOrderOptions.datePart
					: offerOptions.datePart;
		...
		if (this.state.currentView === INVOICE_VIEW) {
			let newDateOptions = dateOptions.map((label) => {
				if (this.state.invoiceIsPeriodic && this.state.invoiceResetFrequency === FREQUENCY_YEAR) {
					label.value === YEAR_KEY || label.value === YEAR_SHORT_KEY
						? (label.disabled = true)
						: (label.disabled = false);
					return label;
				} else if (this.state.invoiceResetFrequency === FREQUENCY_MONTH && this.state.invoiceIsPeriodic) {
					label.value === YEAR_KEY || label.value === YEAR_SHORT_KEY || label.value === FY_KEY
						? (label.disabled = true)
						: (label.disabled = false);
					return label;
				} else if (this.state.invoiceResetFrequency === FREQUENCY_DAY && this.state.invoiceIsPeriodic) {
					label.value === YEAR_KEY ||
					label.value === YEAR_SHORT_KEY ||
					label.value === FY_KEY ||
					label.value === YEAR_MONTH_KEY ||
					label.value === YEAR_SHORT_MONTH_KEY
						? (label.disabled = true)
						: (label.disabled = false);
					return label;
			...
			let newDateOptions = dateOptions.map((label) => {
				if (this.state.purchaseOrderIsPeriodic && this.state.purchaseOrderResetFrequency === FREQUENCY_YEAR) {
					...
					this.state.purchaseOrderResetFrequency === FREQUENCY_MONTH &&
					...
					this.state.purchaseOrderResetFrequency === FREQUENCY_DAY &&
					this.state.purchaseOrderIsPeriodic
				) {
					label.value === YEAR_KEY ||
					label.value === YEAR_SHORT_KEY ||
	...
	onPeriodicSettingChange() {
		...
		if (this.state.currentView === INVOICE_VIEW) {
			...
			this.setState({
				invoiceIsPeriodic: !invoiceIsPeriodic,
				invoiceDate: !invoiceIsPeriodic ? FY_KEY : "",
			...
			this.setState({
				purchaseOrderIsPeriodic: !purchaseOrderIsPeriodic,
				purchaseOrderDate: !purchaseOrderIsPeriodic ? FY_KEY : "",
			...
			this.setState({
				offerIsPeriodic: !offerIsPeriodic,
				offerDate: !offerIsPeriodic ? FY_KEY : "",
	...
	onDateOptionChange(option) {
		...
		if (this.state.currentView === INVOICE_VIEW) {
			...
			this.setState({ offerDateText: dateString });
			if (option.value === "") {
				this.setState({ offerIsPeriodic: false, offerResetFrequency: null, offerResetSubFrequency: null });
			}
		}
		setTimeout(() => {
			if (this.state.currentView === INVOICE_VIEW) {
	...
	onInputNumberChange(event) {
		...
		const maxLength =
			this.state.currentView === INVOICE_VIEW
				? this.state.invoiceCounterLength
				: this.state.currentView === PURCHASE_ORDER_VIEW
				? this.state.purchaseOrderCounterLength
				: this.state.offerCounterLength;
						...
						.then((response) => {
							invoiz
								...
								.then((response) => {
									...
									this.props.numerationOptions.invoice.currentValue = parseInt(
										this.state.invoiceNumber
									);
									this.props.numerationOptions.offer.currentValue = parseInt(this.state.offerNumber);
									this.props.numerationOptions.purchaseOrder.currentValue = parseInt(
								...
								.catch((error) => {
									if (error.body.meta && error.body.meta.increment && error.body.meta.increment[0]) {
										...
										error.body.meta &&
										error.body.meta.startValue &&
										error.body.meta.startValue[0]
									) {
	...
	render() {
		...
		if (!this.isOnlyInvoice && !this.isOnlyOffer && !this.isOnlyPurchaseOrder) {
			...
			if (this.state.currentView === INVOICE_VIEW) {
				error =
					this.state.invoicePrefixError ||
					this.state.invoiceSuffixError ||
					this.state.invoiceNumberError ||
					this.state.incrementByError ||
					this.state.startNumberError;
			} else if (this.state.currentView === PURCHASE_ORDER_VIEW) {
				error =
					this.state.purchaseOrderPrefixError ||
					this.state.purchaseOrderSuffixError ||
					this.state.purchaseOrderNumberError ||
					this.state.incrementByError ||
					this.state.startNumberError;
			} else {
				error =
					this.state.offerPrefixError ||
					this.state.offerSuffixError ||
					this.state.offerNumberError ||
					this.state.incrementByError ||
					this.state.startNumberError;
			}
			let filterItems = [];
				...
				<div className="row u_pb_40">
					...
					<div className="col-xs-8 numeration-content-wrapper">
						<div className="article-history-list-head-content">
							...
							<div className="numeration-error" style={{ marginBottom: 40 }}>
								{error}
							</div>
