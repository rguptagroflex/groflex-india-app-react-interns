...
const POS_RECEIPT_VIEW = 'pos_receipt';
...
const getDefaultInputData=(options)=>{
	...
	let obj={
		...
		number: parseInt(options.currentValue) + parseInt(options.increment) || '',
		suffix: options.suffix || '',
		counterLength: options.counterLength || 4,
		placeHolder1: options.placeHolder1 || '',
		placeHolder2: options.placeHolder2 || '',
		placeHolder3: options.placeHolder3 || '',

		isPeriodic: options.isPeriodic || false,
		increment: options.increment || 1,
		frequency: options.frequency || `daily`,
		subFrequency: options.subFrequency || null,
...
class NumerationConfigComponent extends React.Component {
	constructor(props) {
		...
		const pos_receiptOptions = this.props.numerationOptions.pos_receipt || {};
		...
		this.isOnlyPos_receipt = this.props.isOnlyPos_receipt;
		...
		this.state = {
			...
			//=================== purchaseOrder data end

			


			//============================  pos_receipt data start
			
			pos_receiptPrefix: pos_receiptOptions.prefix || '',
			pos_receiptDate: pos_receiptOptions.datePart || '',
			pos_receiptDateText: '',
			pos_receiptNumber: parseInt(pos_receiptOptions.currentValue) + parseInt(pos_receiptOptions.increment) || '',
			pos_receiptSuffix: pos_receiptOptions.suffix || '',
			pos_receiptCounterLength: pos_receiptOptions.counterLength || 4,
			pos_receiptPlaceholder1: pos_receiptOptions.placeHolder1 || '',
			pos_receiptPlaceholder2: pos_receiptOptions.placeHolder2 || '',
			pos_receiptPlaceholder3: pos_receiptOptions.placeHolder3 || '',

			pos_receiptIsPeriodic: pos_receiptOptions.isPeriodic || false,
			pos_receiptIncrementBy: pos_receiptOptions.increment || 1,
			pos_receiptResetFrequency: pos_receiptOptions.frequency || `daily`,
			pos_receiptResetSubFrequency: pos_receiptOptions.subFrequency || null,
			pos_receiptStartValue: pos_receiptOptions.startValue || 1,

			pos_receiptPrefixError: '',
			pos_receiptSuffixError: '',
			pos_receiptNumberError: '',

			invoicePos_receiptOptions: [],
			isPos_receiptActive: false,




			//============================= pos_receipt data end
			...
			[OFFER_VIEW]:getDefaultInputData(offerOptions),
			[INVOICE_VIEW]:getDefaultInputData(invoiceOptions),
			[PURCHASE_ORDER_VIEW]:getDefaultInputData(purchaseOrderOptions),
			[POS_RECEIPT_VIEW]:getDefaultInputData(pos_receiptOptions),
		...
		setTimeout(() => {
			...
			switch (this.state.currentView) {
				case INVOICE_VIEW:
					datePart=invoiceOptions.datePart
					break;

				case PURCHASE_ORDER_VIEW:
					datePart=purchaseOrderOptions.datePart
					break;

				case OFFER_VIEW:
					datePart=offerOptions.datePart
					break;

				case POS_RECEIPT_VIEW:
					datePart=pos_receiptOptions.datePart
					break;
	...
	disableOptions() {
		const { dateOptions,currentView } = this.state;
		...
		let inputData= this.state[currentView]

		let {isPeriodic,frequency}=inputData

		let newDateOptions = dateOptions.map((label) => {
			if (isPeriodic && frequency === FREQUENCY_YEAR) {
			   (label.value === YEAR_KEY || label.value === YEAR_SHORT_KEY) ? label.disabled = true : label.disabled = false;
			   return label;
		   } else if (frequency === FREQUENCY_MONTH && isPeriodic) {
			   label.value === YEAR_KEY || label.value === YEAR_SHORT_KEY || label.value === FY_KEY ? label.disabled = true : label.disabled = false;
			   return label;
		   } else if (frequency === FREQUENCY_DAY && isPeriodic) {
			   label.value === YEAR_KEY || label.value === YEAR_SHORT_KEY || label.value === FY_KEY || label.value === YEAR_MONTH_KEY || label.value === YEAR_SHORT_MONTH_KEY ? label.disabled = true : label.disabled = false;
			   return label;
		   }  else if (!isPeriodic) {
			   label.value !== FY_KEY ? label.disabled = false : label.disabled = true;
	...
	adjustDigitOptions(adjustBoth) {
		...
		if (adjustBoth || this.state.currentView){
			maxDigits = 10;
			dateKey = this.state.offerDate;
			options = [];
			...
			let inputData= this.state[this.state.currentView]
			inputData.options

			if(inputData.counterLength > maxDigits)
			   inputData.counterLength=maxDigits

			

			if (inputData.number.length > maxDigits) {
				let number =inputData.numbe.slice(inputData.numbe.length - maxDigits);
				number = parseInt(number) < 1 ? '1' : number;

				inputData.number=number
			}

			this.setState({[this.state.currentView]:{...inputData}})
		}
	}
	...
	onFrequencyOptionChange (option) {
		...
		inputData.frequency=option.value
		...
		if (option.value === FREQUENCY_YEAR) {
			...
			inputData.dateText=dateString
			inputData.date=FY_KEY
			this.setState({dateOptions: newDateLabelsOptions})
			...
			inputData.dateText=dateString
			inputData.date=YEAR_SHORT_MONTH_KEY
			...
			inputData.dateText=dateString
			inputData.date=YEAR_SHORT_MONTH_DAY_KEY
			inputData.frequency=null
			this.setState({dateOptions: newDateLabelsOptions})
	...
	onSubFrequencyOptionChange (option) {
		...
		this.setState({ [this.state.currentView]: {...this.state[currentView],subFrequency: option.value} });
	...
	onPeriodicSettingChange () {
		...
		let inputData= this.state[this.state.currentView]
			...
			inputData.isPeriodic= ! inputData.isPeriodic
			inputData.date = inputData.isPeriodic ? FY_KEY : ''
			inputData.dateText = inputData.isPeriodic ? convertDateKeyToPreview(FY_KEY) : ''
			inputData.frequency = inputData.isPeriodic ? FREQUENCY_YEAR : null
			inputData.subFrequency = inputData.isPeriodic ? `January` : null
			inputData.subFrequency=1
	...
	onDateOptionChange(option) {
		...
		let inputData= this.state[this.state.currentView]
		inputData.date=option.value
		inputData.dateText=option.dateString

		if (option.value === '') {

			inputData.isPeriodic=false
			inputData.frequency=null
			inputData.subFrequency=null
		}
		...
		//	}
			...
			let inputData=this.state[this.state.currentView]
					...
					invoiz
					.request(`${config.settings.endpoints.getNumerationSettingDate}${this.state.currentView}/${option.value}`, {
						auth: true,
						method: 'GET'
					})
					.then(response => {
						const { currentValue, increment } = response.body.data;
						inputData.number= parseInt(currentValue) + parseInt(increment)
					...
					.catch(() => {
						inputData.error= parseInt(currentValue) + parseInt(increment)
...
//=========================
		let inputData= this.state[this.state.currentView]
		inputData.counterLength=option.value

			const length = inputData.number && inputData.number.length;
			if (length > option.value) {
				let number =  inputData.number.toString();
				number = number.slice(length - option.value, length);
				number = parseInt(number) < 1 ? '1' : number;
...
//==================
	...
	onPlaceholderOptionChange(option, placeholder) {
		...
		//============
		let inputData= this.state[this.state.currentView]
		inputData[placeholder]=option.value
		this.setState({[this.state.currentView]:{...inputData} });
		//==============
	...
	onTabInputChange(filter) {
		...
		this.setState({currentView:filter.key });
		//==============
		...
		setTimeout(() => {
			...
			this.onDateOptionChange({ value: inputData.date });
			this.onInputNumberBlur();
			this.disableOptions();
		});
	}
	...
	onInputNumberChange(event) {
		...
		let inputData= this.state[this.state.currentView]
		const maxLength = inputData.counterLength
		...
		inputData.number=event.target.value
		inputData.numberError=''
