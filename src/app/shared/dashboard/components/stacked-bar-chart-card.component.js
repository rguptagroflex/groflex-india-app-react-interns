...
class StackedBarChartCardComponent extends React.Component {
	...
	createChart() {
		const { overdue3Days, overdue15Days, overdue45days, overdueAbove45days, totalAmount, chartData, colors } =
			this.state;
	...
	render() {
		...
		return (
			<div className="stacked-bar-chart-card">
				<div className="box">
					<div className="head">
							...
							<div className="row">
								<div className="col-xs-6 body-amount-col" style={{ padding: "0 0.5rem" }}>
									<p className="amount-text">
										{" "}
										<span className="amt-text-indicator-green" /> Total Amount
									</p>
								...
								<div className="col-xs-6 text-right body-overdue-col">
									<p className="amount-text">
										<span className="amt-text-indicator-red" />
										{"Over Due (>3 Days)"}
									</p>
									<p className="text-red">
										{formatCurrencySymbolDisplayInFront(totalAmount - overdue3Days)}
									</p>
								</div>
							</div>
							<hr />
					...
					{demoMode && (
						<div className="empty-placeholder-backdrop text-center">
							<p>{demoText}</p>
							<button
								className="icon icon-sales_outline"
								onClick={() => Invoiz.router.navigate(demoButtonLink)}
							>
								{demoButtonText}
							</button>
						</div>
