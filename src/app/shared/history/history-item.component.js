...
class HistoryItemComponent extends React.Component {
	...
	formatTitle(item) {
		...
		if ((item && item.historyType === HistoryTypes.EMAIL) || item.historyType === HistoryTypes.DOCUMENT) {
			const regex = /\{\{\s*(invoice|offer|deliveryNote|project|recurringInvoice|expense|purchaseOrder|payment|outstandingAmount|email|customer|subject|amount)\s*\}\}/g;
	...
	showEmail(item) {
		if (item.isOwnEmail && !item.invoiceId && !item.deliveryNoteId && !item.offerId && !item.projectId && !item.expenseId && !item.purchaseOrderId) {
			invoiz
