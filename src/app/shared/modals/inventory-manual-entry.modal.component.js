...
class InventoryManualEntryModalComponent extends React.Component {
	constructor(props) {
		...
		this.state = {
			columns: [
				{ active: true, editable: false, label: "Article name", name: "title" },

				{ active: true, editable: false, label: "Action", name: "action" },
				{ active: true, editable: false, label: "Quantity", name: "quantity" },
				// {	active: true,
				// 	editable: false,
				// 	label: "Sender/Receiver",
				// 	name: "customer"
				// },
				{ active: true, editable: false, label: "Date Modified", name: "itemModifiedDate" },
				{ active: true, editable: false, label: "Purchase price", name: "purchasePrice" },
				{ active: true, editable: false, label: "Sales price", name: "salesPrice" },
			],
			positions: [],
			miscOptions: {
	...
	componentDidMount() {
		invoiz.request(`${config.resourceHost}/customer/`, { auth: true, method: "GET" }).then(({ body: { data } }) => {
			...
			<div className="inventory-manual-entry-modal">
				<div className="has-footer ">
					<div className="user-invite-modal-content" style={{ height: 800 }}>
						...
						<span style={{ fontWeight: 600, fontSize: 13 }}>Note: </span>
						<span style={{ fontSize: 13 }}>
							Adding untracked articles will automatically track them in stock movement
						</span>
						<div className="inventory-manual-body">
							...
							{!this.state.isLoading ? (
								<LetterPositionsInventoryManualComponent
									columns={this.state.columns}
									positions={this.state.positions}
									onPositionsChanged={(positions) => this.onLetterPositionsChange(positions)}
									resources={resources}
