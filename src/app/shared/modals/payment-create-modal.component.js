...
const getTotalPreviousBalance = (customer) => customer ? Math.abs(customer.credits + customer.balance + (customer.openingBalance < 0 ? customer.openingBalance : 0 )) : 0
class PaymentCreateModalComponent extends React.Component {
	...
	save() {
		const { isSaving, totalAmount,isUsingPreviousBalance,isClearingDues,paymentAmount, usingPreviousBalance} = this.state;
	...
	hanldeAmountChange(value){
		let  {paymentAmount,totalAmount,requiredPaymentAmount ,usingPreviousBalance,  isClearingDues,clearingDues ,isNormalPayment} = this.state
		let {payment} = this.props
			...
			this.setState({ paymentAmount,totalAmount, isClearingDues,clearingDues,isNormalPayment,requiredPaymentAmount });
	}	
	handlePreviousBalanceCheckbox(){
		
		let {
			isUsingPreviousBalance, previousBalance,usingPreviousBalance, reamingPreviousBalance,
			 paymentAmount,totalAmount,isNormalPayment,requiredPaymentAmount }=this.state
		...
		if( isUsingPreviousBalance){
			...
			if(usingPreviousBalance > previousBalance )
				usingPreviousBalance = previousBalance
			if(usingPreviousBalance > requiredPaymentAmount)
				usingPreviousBalance = requiredPaymentAmount
		...
		totalAmount= parseFloat(accounting.toFixed(paymentAmount + usingPreviousBalance,2))
		isNormalPayment = totalAmount === requiredPaymentAmount
		this.setState({isUsingPreviousBalance,usingPreviousBalance ,reamingPreviousBalance,totalAmount,paymentAmount,isNormalPayment })
	}
	handleUsingPreviousBalanceChange(value){
		let {usingPreviousBalance, totalAmount,reamingPreviousBalance, previousBalance,paymentAmount, isNormalPayment, requiredPaymentAmount } = this.state	
				usingPreviousBalance = accounting.unformat(value, config.currencyFormat.decimal);
				if(usingPreviousBalance < 0)
				usingPreviousBalance = 0
	...
	hanldeClearDuesCheckbox(){
		let {totalDues,isClearingDues,clearingDues,paymentAmount,totalAmount,requiredPaymentAmount,isNormalPayment, usingPreviousBalance}=this.state
		let {payment} = this.props
		...
		this.setState({isClearingDues,clearingDues, totalAmount,paymentAmount,isNormalPayment,requiredPaymentAmount})
	}
	...
	createPaymentView() {
		...
		let { isNormalPayment, isSaving,paymentAmount,isUsingPreviousBalance,previousBalance,usingPreviousBalance,totalAmount,isClearingDues,clearingDues,totalDues,hasDues } = this.state;
							...
							<div className ='inline-checkbox-curreny-input ' >
								...
								<div className='curreny-input' >
									<CurrencyInputComponent
										...
										onBlur={value => {
											if (value!=='') 
												this.handleClearingDuesBalanceChange(value)
											}
										}
										currencyType={`symbol`}
									/>
...

{
							...
							<div className ='inline-checkbox-curreny-input ' >
								...
								<div className='curreny-input' >
									<CurrencyInputComponent
										...
										onBlur={value => {
											if (value!=='') 
												this.handleUsingPreviousBalanceChange(value)
											}
										}
										currencyType={`symbol`}
									/>
								</div>
								<div className='post-text' >{`of the previous balance ${formatCurrency(previousBalance )}`} </div>
							</div>
						 </div>	
						)
						:(
	...
	
				{
					...
					<div className='row' >
							<div className="col-xs-6">
								<div className="payment-create-value">{`Total payment amount: ${ formatCurrency(totalAmount)}`}</div>
							</div>
					</div>
				}
	...
	createDeviationView() {
		...
		let messageText = messageText=`The excess payment of ${formatCurrency( headerDisplayAmount  )} can be used for upcoming invoices`

		let deviationType = 'MORE' 

		if(totalAmount > requiredPaymentAmount )
			deviationType = 'MORE'
		...
		const isMore = deviationType.includes('MORE')
		 ...
		 switch (deviationType) {
			 case 'MORE':{
				...
				messageText=`The excess payment of ${formatCurrency( headerDisplayAmount  )} can be used for upcoming invoices`
				break;
			 }
		...
		return (
			<div>
				<div className="modal-base-headline">
					...
					<span className="deviation-amount">{formatCurrency(headerDisplayAmount)}{ !isMore ? <span style={{color: 'black'}}>{` as: `}</span> : ``}</span>
						...
						<ButtonComponent
							...
							callback={() => {
								if (isDeviationSet) {
									...
									this.setDeviation(invoice.baseCurrency && invoice.exchangeRate ? PAYMENT_TYPE.EXCHANGE_GAIN : PAYMENT_TYPE.EXCESS);
									this.save();
								}
							}}
