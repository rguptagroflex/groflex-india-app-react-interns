...
class ChangeUserRoleModal extends React.Component {
	...
	render() {
		...
		return (
			<div className='has-footer change-user-role-modal'>{this.state.step === 0 ? (
				<div>
					<div className='change-user-role-modal-headline text-semibold'>Change role</div>
					<div className='change-user-role-modal-subheadline'>Determine the permissions for the user <span className='text-semibold'>{firstName} {lastName}</span></div>
					<RoleSelectComponent onRadioChange={this.onRadioChange} selectedRole={this.state.selectedRole === '' ? userInitRole : this.state.selectedRole} isInvite={false}/>
				</div>
			) : (
				<div>
					<div className='change-user-role-modal-headline text-semibold'>Make the owner</div>
					<div className='change-user-role-modal-subheadline'>Are you sure that you {firstName} {lastName} want to make the owner of this invoiz account? You cannot change this afterwards.</div>
				</div>
			)}
