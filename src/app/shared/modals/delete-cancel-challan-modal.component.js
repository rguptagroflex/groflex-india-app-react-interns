...
class DeleteCancelChallanModalComponent extends React.Component {
	constructor(props) {
		super(props);
	...
	render() {
		...
		const { challan, resources } = this.props;

		const cancelElement = (
			<div>
				<div className="modal-base-headline"> {format(resources.challanCancelHeading, challan.number)}</div>
		...
		const deleteElement = (
			<div>
				...
				<div className="modal-base-headline">{format(resources.challanDeleteHeading, challan.number)}</div>
				<div className="delete-invoice-warning">
					<div className="delete-invoice-warning-icon">!</div>
					<div className="delete-invoice-warning-text">{resources.challanCompletedMessage}</div>
				</div>

				<CheckboxInputComponent
					name={"confirmDelete"}
					label={resources.deleteChallanWarningMessage}
		...
		this.setState({ isSaving: true }, () => {
			invoiz
				...
				.then(() => {
					...
					invoiz.page.showToast({ message: resources.challanCancelSuccessMessage });
	...
	deleteInvoice() {
		...
		this.setState({ isSaving: true }, () => {
			invoiz
				...
				.then(() => {
					updateSubscriptionDetails();
					invoiz.page.showToast({ message: resources.challanDeleteConfirmationMessage });
					if (this.props.isFromList) {
						invoiz.router.reload();
					} else {
						invoiz.router.navigate("/challan");
					}
					ModalService.close();
				})
				.catch(() => {
...
export default DeleteCancelChallanModalComponent;
