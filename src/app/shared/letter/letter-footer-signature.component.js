...
class LetterFooterSignatureComponent extends React.Component {
	...
	render() {
		...
		switch (this.state.footerState) {
					...
					switch (type) {
						...
						case 'text':
							...
							if (font === DEFAULT_FONT_FAMILY) {
								return (
									<div className="footerSignatureName" key={`letter-footer-item-${index}`} dangerouslySetInnerHTML={{ __html: html }}></div>
								);
							}
					}
				});
			...
			case LetterFooterState.EDIT:
				...
				const uploadButton = (
					<div className="signatureEmpty" >
						<div className="icon icon-rounded icon-plus" />
						<div className="signatureEmpty_label" ref="uploadButton" >{resources.letterFooterSignatureUploadLogoText}</div>
					</div>
				);
				content = (
					<div>
						...
						<div
							className={`footerEdit ${
								this.state.wrapperVisible ? 'footerEdit-active' : ''
							}`}
						>
							<div>
								...
								{this.state.showClearButton === false ? <div className="orSection">{resources.letterFooterSignatureTextTitle}</div> : null}
							</div>
							{this.state.imageSelected === false ? <div className="signature-text-div">
								<input type="text"
							...
							<div className="canvas-upper-div"><canvas ref="canvas" /></div>
						</div>
					</div>
				);
				break;
		...
		return (
			<div className="footerSignatureItems" onClick={event => this.onFooterWrapperClick(event)}>
				...
				<button className={`button-icon-close signatureClose_btn ${this.state.items.length > 1 || this.state.showClearButton ? 'signatureClose' : ''}`} onClick={() => this.deleteObject()} />
	...
	onDocumentClick(event) {
		if ($(event.target).closest('.footerSignatureItems').length === 0 && !this.isSaving && this.footerStateEdit === true) {
			this.isSaving = true;
			this.footerStateEdit = false;
	...
	deleteObject() {
		...
		this.setState({ deleteButtonActive: false,
			showClearButton: false,
			imageSelected: false
		});
		this.setState({ footerState: LetterFooterState.EDIT }, () => {
