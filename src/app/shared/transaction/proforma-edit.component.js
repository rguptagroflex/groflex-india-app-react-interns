...
class ProformaEditComponent extends React.Component {
	constructor(props) {
		super(props);
	...
	render() {
		...
		const topbar = (
			<TopbarComponent
				title={this.props.isEdit ? "Edit Proforma Invoice" : "Create Proforma Invoice"}
		...
		new Promise((resolve, reject) => {
			if (isProject) {
				...
				saveOffer(requestData).then(
					(id) => {
						amplitude.getInstance().logEvent("created_proforma");
						redirectUrl = `proformaInvoice/${id}`;
						resolve();
					},
					(error) => {
						reject(error);
...
export default connect(mapStateToProps, mapDispatchToProps)(ProformaEditComponent);
