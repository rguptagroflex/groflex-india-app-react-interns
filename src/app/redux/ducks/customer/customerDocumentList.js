...
const initialState = {
	...
	errorOccurred: false,
	columns: [
		{ key: "date", title: "Datum", width: "100px", resourceKey: "date" },
		{ key: "type", title: "Vorgang", width: "100px", resourceKey: "operation" },
		{ key: "number", title: "Nr.", width: "85px", resourceKey: "serialNumber" },
		{ key: "state", title: "State", width: "100px", resourceKey: "status" },
		{ key: "totalGross", title: "Brutto", width: "100px", align: "right", resourceKey: "gross" },
	],
	currentPage: 1,
	totalPages: 1,
	filterItems: [
	...
	switch (action.type) {
		...
		case UPDATE_FILTER_ITEMS: {
			...
			const newFilterItems = filterItems.filter((filterItem) => {
				return customerType === `payee`
					? filterItem.key !== "offer" && filterItem.key !== `invoice` && filterItem.key !== `credits`
					: filterItem.key !== `expense` && filterItem.key !== `purchaseOrder`;
			});
